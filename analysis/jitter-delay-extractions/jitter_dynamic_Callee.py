...
RTP_CLOCK_RATE = 48000     # Common for OPUS; adjust if needed for other codecs
...
def main():
    ...
    with tqdm(total=len(pcap_files), desc="Processing PCAPs", unit="file") as pbar:
        for pcap_file in pcap_files:
            ...
            # Extract arrival time & RTP timestamp for each RTP packet
            ...
            for pkt in packets:
                if IP in pkt and UDP in pkt:
                    ...
                    try:
                        ...
                        _, _, _, _, _, payload_type, _, rtp_ts, _ = rtp

                        # Only process packets with dynamic payload types for OPUS
                        if payload_type in range(96, 128):  # Dynamic payload types
