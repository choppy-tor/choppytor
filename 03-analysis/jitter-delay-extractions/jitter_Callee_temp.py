...
# Configuration
# ------------------------------------------------------------------------------
# PCAP_DIR = "/home/jithin/VoIP-again-Tor/data/G711/12-20-24/12-20-24/loggr/1Frank-Lond/callee/data/test_pcaps"
PCAP_DIR = sys.argv[1]
# PCAP_DIR = input("Enter the directory where your PCAPs are located: ").strip()
SOURCE_IP = "10.8.0.2"     # Optional filter on source IP, set to None if unused
...
# ------------------------------------------------------------------------------
def main():
    pcap_pattern = os.path.join(PCAP_DIR, "pcap_callee_inside_*.pcap")
    #pcap_pattern = os.path.join(PCAP_DIR, "pcap_callee_callee_inside_*.pcap")
    ...
    with tqdm(total=len(pcap_files), desc="Processing PCAPs", unit="file") as pbar:
        for pcap_file in pcap_files:
            ...
            try:
                # Attempt to read packets from the PCAP file
                ...
                pbar.update(1)
                continue
            # 2. Read all packets from the current pcap
            # packets = rdpcap(pcap_file)

            # 3. Extract arrival time & RTP timestamp for each RTP packet
            ...
            # 4. Compute jitter using RFC 3550 method
            ...
            for i in range(1, len(arrival_and_timestamp)):
                ...
                delta = current_transit - prev_transit
                delta = abs(delta)
                J += (delta - J) / 16.0
