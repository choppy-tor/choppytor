            ...
            for stream in controller.get_streams():
                if stream.status == 'SUCCEEDED' and stream.target_port == port:
                    if stream.circ_id == circ.id:
                        guard_node_fingerprint = circ.path[0][0]  # Get the fingerprint of the guard node
                        guard_node = controller.get_network_status(guard_node_fingerprint)
#                        middle_node_fingerprint = circ.path[1][0]  # Get the fingerprint of the guard node
#                        middle_node = controller.get_network_status(middle_node_fingerprint)
#                        exit_node_fingerprint = circ.path[2][0]  # Get the fingerprint of the guard node
