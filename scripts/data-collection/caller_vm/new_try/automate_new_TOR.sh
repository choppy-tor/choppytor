...
for ((i=0; i<n; i++)); do
    ...
    if [ -f "data/test_caller_recordings/caller_recording_$i.wav" ]; then
        caller_pesq_raw=$(./pesq +8000 Julius2.wav ~/new_try/data/test_caller_recordings/caller_recording_$i.wav | grep "Raw MOS" | awk '{print $7}' )
        caller_pesq_mos=$(./pesq +8000 Julius2.wav ~/new_try/data/test_caller_recordings/caller_recording_$i.wav | grep "Raw MOS" | awk '{print $8}')
    fi
