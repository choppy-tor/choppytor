...
rm data.zip
rm -r /root/new_try/data/test_pcaps /root/new_try/data/test_caller_recordings /root/new_try/data/test_owd_logs

mkdir -p /root/new_try/data/test_pcaps /root/new_try/data/test_caller_recordings /root/new_try/data/test_owd_logs
...
for ((i=0; i<n; i++)); do
    ...
    echo "$rtt_min $rtt_avg $rtt_max $rtt_mdev" > /root/new_try/data/owd_logs/owd_$i.txt
    ...
    echo "Guard Node IP: $guard_ip, Exit Node IP: $exit_ip"

    if [[ -n "$guard_ip" && -n "$exit_ip" ]]; then
        ...
        killall tcpdump
        pkill python2.7
        mv caller_recording.wav /root/new_try/data/test_caller_recordings/caller_recording_$i.wav
    fi
...
#    tor
    ...
    if [ -f "data/test_caller_recordings/caller_recording_$i.wav" ]; then
        caller_pesq_raw=$(./pesq +8000 Julius2.wav /root/new_try/data/test_caller_recordings/caller_recording_$i.wav | grep "Raw MOS" | awk '{print $7}' )
        caller_pesq_mos=$(./pesq +8000 Julius2.wav /root/new_try/data/test_caller_recordings/caller_recording_$i.wav | grep "Raw MOS" | awk '{print $8}')
    fi
    ...
    json_entry=$(jq -n \
        ...
        '{
            ...
            timestamp: $timestamp,
            path: {guard_node: $guard_ip, exit_node: $exit_ip},
