...
rm data.zip
rm -r ~/new_try/data/test_pcaps ~/new_try/data/test_caller_recordings ~/new_try/data/test_owd_logs

mkdir -p ~/new_try/data/test_pcaps ~/new_try/data/test_caller_recordings ~/new_try/data/test_owd_logs
...
for ((i=0; i<n; i++)); do
    ...
    if [[ -n "$guard_ip" && -n "$middle_ip" && -n "$exit_ip" ]]; then
        ...
        # out_tcp_dump_pid=$!
        # python2.7 $CALLER_SCRIPT caller_recording 10.8.0.1 9001 1009 1234 1001 45 8000 Julius2_v2.wav "$TUNNEL_IP"
        python2.7 $CALLER_SCRIPT caller_recording 10.8.0.1 9001 1011 1234 1001 50 8000 "$TUNNEL_IP"
        sleep 4
        # kill $in_tcp_pid 2>/dev/null
        killall tcpdump
        pkill python2.7
        mv caller_recording.wav ~/new_try/data/test_caller_recordings/caller_recording_$i.wav
    fi
    ...
    if [ -f "data/test_caller_recordings/caller_recording_$i.wav" ]; then
        caller_pesq_raw=$(./pesq +8000 Julius2_v2.wav ~/new_try/data/test_caller_recordings/caller_recording_$i.wav | grep "Raw MOS" | awk '{print $7}' )
        caller_pesq_mos=$(./pesq +8000 Julius2_v2.wav ~/new_try/data/test_caller_recordings/caller_recording_$i.wav | grep "Raw MOS" | awk '{print $8}')
    fi
